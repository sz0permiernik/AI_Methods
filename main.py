...
csvData = excelData.to_csv('dane_rzeczywiste.csv', index=False, header=False)
...
def testing_for_ImplementedAdasyn(x, y, method, skfold, data):
    ...
    method_x, method_y = method._fit_resample(x, y)
    ...
    for i, (train_index, test_index) in enumerate(skfold.split(x, y)):
        ...
        x_train_resample, y_train_resample = method._fit_resample(x_train, y_train)
...
testing_for_ImplementedAdasyn(x, y, iada, skf, synt)
testing(x, y, ada, skf, synt)
testing(x, y, sm, skf, synt)
testing(x, y, br, skf, synt)

# Testing methods for real data
testing_for_ImplementedAdasyn(real_x, real_y, iada, skf, real)
